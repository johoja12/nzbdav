...
| Commit | Description | Priority | Approach | Conflicts |
|--------|-------------|----------|----------|-----------|
| `23d8541` | Fixed `Cannot find byte position` bug | P1 | Cherry-pick | None - affects NzbFileStream.cs |
| `7966414` | Fixed bug determining rar part numbers | P1 | Cherry-pick | Low - RarAggregator/RarProcessor changes |
| `686878b` | Added support for par2 filenames containing relative paths | P1 | Cherry-pick | None |
| `7953211` | Added additional validation for rar volumes | P2 | Cherry-pick | Low |
| `aca8eac` | Fixed bug in GetQueueController when requesting non-first page | P2 | Cherry-pick | None |
| `0cca3dd` | Fixed bug when updating upload-category dropdown while uploads active | P2 | Cherry-pick | None |
| `326be3f` | Fixed identifying root-cause exceptions | P2 | Cherry-pick | None |

### 1.2 Connection/Network Fixes

| Commit | Description | Priority | Approach | Conflicts |
|--------|-------------|----------|----------|-----------|
| `e46125e` | Do not retry nntp command when cancellation is requested | P1 | Manual | **HIGH** - We have custom MultiConnectionNntpClient changes |
| `9a66a24` | Ensure backend-proxied connections are closed correctly on errors | P2 | Cherry-pick | Low - frontend/server/app.ts |
| `b51ae7e` | Added logic to replace unhealthy Nntp connections | P2 | Review | Medium - check against our ConnectionPool |
...
| `9f12495` | Added pagination component | P3 | Cherry-pick | We have pagination too - review |
...
| Commit | Description | Priority | Approach | Conflicts |
|--------|-------------|----------|----------|-----------|
| `14e6b6e` | Update entrypoint.sh to handle existing user/group of PUID/PGID | P2 | Manual | **We already have this fix** - compare |
